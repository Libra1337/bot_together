...
_FUZZY_COMMAND_SPECS = [
    ...
    },
    {
        ...
        "exacts": {"/restart"},
        "prefixes": ["/restart "],
        "info": "指令说明：`/restart now`、`/restart -5`、`/restart -c`\n用途：立即重启、定时重启或取消计划重启。",
    },
    {
        "command": "/check",
        "exacts": {"/check"},
...
# Admin 授权用户集合（通过 /auth 验证后才能使用 /nfa、/restart 等管理指令）
...
def _load_admin_authorized() -> set[int]:
    ...
    try:
        ...
        _log.warning(f"[AUTH] 加载管理员授权文件失败: {e}")
    return set()
...
def _load_nfa_user_authorized() -> set[int]:
    """从文件加载可使用 /nfa 的个人白名单"""
    try:
...
async def handle_command(
    ...
    if content.startswith("/auth ") or content.startswith("/auth:"):
        ...
        if message_type == "private" and code == ADMIN_AUTH_CODE:
            ...
            await send_reply(
                ...
                user_id,
                group_id,
                "✅ 登陆成功喵~您现在可以使用管理指令了！（/nfa、/restart 等）\n授权已持久化，重启后无需重新验证~",
            )
    ...
    if _ad_match:
        ...
        if action == "s-":
            ...
            return True
            new_id = max((int(ad.get("id", 0)) for ad in _ads), default=0) + 1
            active_until = _time_mod.time() + duration if duration else None
            _ads.append(
                {
                    "id": new_id,
                    "content": ad_content,
    ...
    if content.startswith("/restart") or content.startswith("restart"):
        if not _is_admin_or_staff(user_id):
            await send_reply(
                ws,
                message_type,
        ...
        if param.startswith("-"):
            try:
                ...
                await send_reply(
                    ...
                    user_id,
                    group_id,
                    "格式错误喵！用法：/restart -分钟数（如 /restart -5）",
                )
                return True
            ...
            await send_reply(
                ...
                user_id,
                group_id,
                f'🔄 计划于 {time_str} 重启，使用"/restart -c"以取消。',
            )
        ...
        await send_reply(
            ...
            user_id,
            group_id,
            "用法：/restart now | /restart -分钟数 | /restart -c",
        )
        return True
    ...
    if message_type == "group":
        ...
        if ac_cmd in ("开空调", "/开空调"):
            ...
            # 升温/降温 支持自定义度数：空调升温 10 或 空调升温10
                    ...
                    async for raw in ws:
                        ...
                        if post_type == "notice":
                            ...
                            if notice_type == "group_increase":
                                ...
                                _log.info(
                                    f"[撤回提醒] 群{gid} {recall_nickname} 撤回: {recall_content[:50]}"
                                )
                                ...
                                _log.info(
                                    f"[撤回提醒] 私聊 {recall_nickname} 撤回: {recall_content[:50]}"
                                )
